...
------------------------------------
Courses page — reads data from MongoDB courses collection.
"""
...
def _extract_course_fields(doc):
    """Extract course fields from a MongoDB document (payload or top-level)."""
    payload = doc.get("payload", {})
    result = {}
    for field in COURSE_FIELDS:
        # Check payload first, then top-level
...
def _fetch_courses() -> list[dict]:
    """Fetch all rows from the courses collection in MongoDB."""
    from app.utils.mongo import get_upload_collection
    col = get_upload_collection("courses")

    rows = []
    for doc in col.find({}).sort([("payload.level", 1), ("payload.short_name", 1), ("payload.course_name", 1)]):
        rows.append(_extract_course_fields(doc))
...
def courses():
    """Main courses page — fetches live data from MongoDB."""
    try:
