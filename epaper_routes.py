...
def _compute_newspaper_layout(images, order):
    """
    Pixel-perfect newspaper layout.
    order[0] = hero (full-width, top). Remaining go in 2–3 column rows.
    Each block height = round(col_width × img_h / img_w) — exact aspect ratio,
    so object-fit:contain fills the block with ZERO white bars and ZERO cropping.
    """
    W, H, G = 800, 1131, 5

    def exact_h(img, w):
        iw = max(1, int(img.get("w", 800)))
        ih = max(1, int(img.get("h", 600)))
        return max(40, round(w * ih / iw))

    layout = [None] * len(images)
    remaining = list(order)
    y = 0

    # Hero: full width, aspect-ratio height, capped at 48 % of canvas
    if remaining:
        idx = remaining.pop(0)
        bh = min(exact_h(images[idx], W), round(H * 0.48))
        layout[idx] = {"x": 0, "y": y, "w": W, "h": bh}
        y += bh + G

    # Rest: rows of up to 3 equal-width columns
    while remaining and y < H - 40:
        row = remaining[:3]
        remaining = remaining[3:]
        cols = len(row)
        col_w = (W - G * (cols - 1)) // cols
        row_max_h = 0
        for ci, idx in enumerate(row):
            bh = min(exact_h(images[idx], col_w), H - y)
            layout[idx] = {"x": ci * (col_w + G), "y": y, "w": col_w, "h": bh}
            row_max_h = max(row_max_h, bh)
        y += row_max_h + G

    # Overflow (>6 or canvas full): stack remaining full-width
    for idx in remaining:
        if y < H - 40:
            bh = min(exact_h(images[idx], W), H - y)
            layout[idx] = {"x": 0, "y": y, "w": W, "h": bh}
            y += bh + G
...
@epaper_bp.route("/api/epaper/admin/ai-layout", methods=["POST"])
def api_ai_layout():
    """
    Groq AI decides which image is the hero and ranks the rest by importance.
    Python then computes pixel-perfect newspaper positions from that ordering.
    """
    data = request.get_json(silent=True) or {}
    images = data.get("images", [])   # [{url, w, h}, ...]

    if not images or not (1 <= len(images) <= 6):
        return jsonify({"error": "Provide 1–6 images"}), 400

    n = len(images)

    # Default ordering: largest pixel area first (most prominent = hero)
    order = sorted(range(n), key=lambda i: int(images[i].get("w", 0)) * int(images[i].get("h", 0)), reverse=True)

    # Ask Groq to improve the ordering if API key is available
    api_key = os.getenv("GROQ_API_KEY")
    if api_key and n > 1:
        try:
            img_desc = "\n".join(
                f"Image {i} (0-based): {int(img.get('w',0))}×{int(img.get('h',0))} px  "
                f"(area {int(img.get('w',0))*int(img.get('h',0))})"
                for i, img in enumerate(images)
            )
            prompt = (
                f"You are a newspaper editor. Rank these {n} images for a front-page layout.\n"
                f"The most important image becomes the full-width hero at the top.\n\n"
                f"{img_desc}\n\n"
                f"Return ONLY a JSON array of 0-based indices, most important first.\n"
                f"Example for 3 images: [1, 0, 2]\n"
            ...
            if m:
                ...
                if (len(parsed) == n
                        and all(isinstance(x, int) and 0 <= x < n for x in parsed)
                        and len(set(parsed)) == n):
                    order = parsed
        except Exception:
            pass  # keep area-based default ordering

    layout = _compute_newspaper_layout(images, order)
